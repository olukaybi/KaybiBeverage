...
function buildWhatsAppMessage(r: Result): string {
  ...
  const lines = [
    ...
    '',
    `Estimated total: ₦${r.subtotal.toLocaleString('en-NG')}`,
    '',
        ...
        {result && (
          ...
          >
            ...
            <div className="bg-kayora-blue-900 rounded-xl px-6 py-5 mb-4 text-center">
              <p className="text-kayora-cream/70 text-sm mb-1">Estimated total</p>
            ...
            <p className="text-xs text-kayora-stone mb-6 leading-relaxed">
              Bulk and event orders may qualify for attractive rebates and other incentives — request a quote for distributor terms.
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <Link
                href={`/shop${result.jars189L > 0 ? '#18.9L' : '#50cl'}`}
              ...
              >
                Order This for My Event &rarr;
              </Link>
              <a
                href={buildWhatsAppLink(buildWhatsAppMessage(result))}
                target="_blank"
            ...
            <Link
              href={`/contact?reason=bulk%2Fevent+order&event_type=${result.inputs.eventType}&guests=${result.inputs.guests}&days=${result.inputs.days}&hot=${result.inputs.hotBump}#bulk-quote`}
              className="mt-3 block text-center text-xs text-kayora-stone hover:text-kayora-blue-700 hover:underline"
            >
              Prefer a written quote? Request one for your event &rarr;
            </Link>
          </div>
        )}
