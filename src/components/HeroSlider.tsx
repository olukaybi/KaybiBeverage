...
interface Slide {
  id: string;
  type: 'product' | 'lifestyle' | 'factory';
...
const slides: Slide[] = [
  ...
  },
  {
    id: 'sharp-sharp',
    type: 'product',
    backgroundImage: '/images/slider/lifestyle-event.jpg',
    mobileBackgroundImage: '/images/slider/lifestyle-event-mobile.jpg',
  ...
  },
  {
    id: 'original',
    type: 'product',
    backgroundImage: '/images/slider/lifestyle-everyday.jpg',
    mobileBackgroundImage: '/images/slider/lifestyle-everyday-mobile.jpg',
  ...
  },
  {
    id: 'jara',
    type: 'product',
    backgroundImage: '/images/slider/lifestyle-fitness.jpg',
    mobileBackgroundImage: '/images/slider/lifestyle-fitness-mobile.jpg',
    eyebrow: 'Jara · 75cl',
  ...
  },
  {
    id: 'never-finish',
    type: 'product',
    backgroundImage: '/images/slider/lifestyle-office.jpg',
    mobileBackgroundImage: '/images/slider/lifestyle-office-mobile.jpg',
  ...
  },
  {
    id: 'factory',
    type: 'factory',
    backgroundImage: '/images/slider/factory.jpg',
    mobileBackgroundImage: '/images/slider/factory-mobile.jpg',
  ...
  return (
    ...
    >
      <AnimatePresence mode="sync">
        ...
        >
          <Image src={slide.mobileBackgroundImage ?? slide.backgroundImage} alt="" fill priority={current === 0} className="object-cover object-center lg:hidden" sizes="100vw" />
          <Image src={slide.backgroundImage} alt="" fill priority={current === 0} className="hidden object-cover object-center lg:block" sizes="100vw" />
          <div className="absolute inset-0" style={{ background: `rgba(10,28,54,${slide.overlayStrength ?? 0.5})` }} aria-hidden="true" />
        </motion.div>
      </AnimatePresence>
        ...
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 lg:py-32">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="max-w-2xl"
            >
              <p className="text-xs uppercase tracking-widest text-kayora-gold-500 font-sans mb-4">{slide.eyebrow}</p>
              <h1 className="font-display text-display-lg text-white mb-5 whitespace-pre-line leading-tight">{slide.headline}</h1>
              <p className="text-base lg:text-lg leading-relaxed text-white/80 mb-8 max-w-[55ch]">{slide.subhead}</p>
              <div className="flex flex-col sm:flex-row gap-4">
                <Link href={slide.primaryCTA.href} className="inline-flex items-center justify-center min-h-[48px] px-8 py-3 bg-kayora-blue-900 text-white font-semibold rounded-lg hover:bg-kayora-blue-700 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2">
                  {slide.primaryCTA.label}
                </Link>
                {slide.secondaryCTA && (
                  <Link href={slide.secondaryCTA.href} className="inline-flex items-center justify-center min-h-[48px] px-8 py-3 border border-white/60 text-white font-semibold rounded-lg hover:bg-white/10 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2">
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 flex gap-2" role="tablist" aria-label="Slide indicators">
        {slides.map((s, i) => (
          <button key={s.id} role="tab" aria-selected={i === current} aria-label={`Go to slide ${i + 1}`} onClick={() => goTo(i)}
      ...
      {!paused && (
        <div className="absolute bottom-0 left-0 z-20 h-[3px] bg-kayora-gold-500/30 w-full">
          <motion.div key={slide.id + '-progress'} className="h-full bg-kayora-gold-500" initial={{ width: '0%' }} animate={{ width: '100%' }} transition={{ duration: AUTOPLAY_INTERVAL / 1000, ease: 'linear' }} />
        </div>
      )}
    </section>
  );
